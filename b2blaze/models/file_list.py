...
class B2FileList(object):
    ...
    def _update_files_list(self, retrieve=False, limit=None):
        ...
        params = {"bucketId": self.bucket.bucket_id, "maxFileCount": limit or 10000}
        while new_files_to_retrieve:
            ...
            if response.status_code == 200:
                ...
                for file_json in files_json["files"]:
                    new_file = B2File(
                        connector=self.connector, parent_list=self, **file_json
                    )
    ...
    def all_file_versions(self, limit=None):
        ...
        file_ids = []
        new_files_to_retrieve = True
        params = {"bucketId": self.bucket.bucket_id, "maxFileCount": 10000}
    ...
    def _get_by_name(self, file_name):
        ...
        path = API.list_all_files
        params = {"prefix": b2_url_encode(file_name), "bucketId": self.bucket.bucket_id}
        ...
        if not len(file_json["files"]) > 0:
            ...
            return B2File(
                connector=self.connector, parent_list=self, **file_json["files"][0]
            )
    ...
    def upload(
        ...
        params = {"bucketId": self.bucket.bucket_id}
        upload_url_response = self.connector.make_request(
            path=get_upload_url_path, method="post", params=params
        )
        if upload_url_response.status_code == 200:
            ...
            upload_response = self.connector.upload_file(
            ...
            if upload_response.status_code == 200:
                new_file = B2File(
                    connector=self.connector, parent_list=self, **upload_response.json()
                )
            ...
            def upload_part_worker(args):
                ...
                with open(contents.name, "rb") as file:
                    ...
                    upload_part_url_response = self.connector.make_request(
                        path=get_upload_part_url_path, method="post", params=params
                    )
